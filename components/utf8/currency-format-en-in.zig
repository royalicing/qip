// Format one exact ASCII decimal using en-IN currency conventions.
...
const currency_data = @import("lib/currency-format-en-in-table.zig");

// The source table stays readable while the emitted representation fits each
// lookup entry in one u32. Most en-IN prefixes are the alphabetic code plus a
...
test "currency uniform resets to USD after render" {
    ...
    const input = "1234.5";
    const expected_eur = "€1,234.50";
    const expected_usd = "$1,234.50";
