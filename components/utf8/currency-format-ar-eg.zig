// Format one exact ASCII decimal using ar-EG currency conventions.
...
// lookup entry in one u32. Most ar-EG suffixes are a non-breaking space plus
...
fn writeArabicDigit(out: *usize, digit: u8) void {
    output_buf[out.*] = 0xd9;
    output_buf[out.* + 1] = 0xa0 + (digit - '0');
    out.* += 2;
}
...
export fn render(input_size_in: u32) u32 {
    ...
    var out: usize = 0;
    const sign_prefix = if (amount.negative) "\xD8\x9C-\xE2\x80\x8F" else "\xE2\x80\x8F";
    ...
    for (integer_buf[0..integer_len], 0..) |digit, i| {
        if (i > 0 and (integer_len - i) % 3 == 0) {
            output_buf[out] = 0xd9;
            output_buf[out + 1] = 0xac;
            out += 2;
        }
        writeArabicDigit(&out, digit);
    }
    if (currency.fraction_digits > 0) {
        output_buf[out] = 0xd9;
        output_buf[out + 1] = 0xab;
        out += 2;
        var divisor: u16 = 1;
        position = 1;
