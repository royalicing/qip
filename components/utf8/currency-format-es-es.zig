// Format one exact ASCII decimal using es-ES currency conventions.
...
const currency_data = @import("lib/currency-format-es-es-table.zig");

// The source table stays readable while the emitted representation fits each
// lookup entry in one u32. Most es-ES suffixes are a non-breaking space plus
...
export fn render(input_size_in: u32) u32 {
    ...
    for (integer_buf[0..integer_len], 0..) |digit, i| {
        if (integer_len >= 5 and i > 0 and (integer_len - i) % 3 == 0) {
            output_buf[out] = '.';
            out += 1;
        }
...
test "currency uniform resets to USD after render" {
    ...
    const input = "1234.5";
    const expected_eur = "1234,50\u{a0}€";
    const expected_usd = "1234,50\u{a0}US$";
