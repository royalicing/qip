// Format one exact ASCII decimal using ja-JP currency conventions.
...
const currency_data = @import("lib/currency-format-ja-jp-table.zig");

// The source table stays readable while the emitted representation fits each
// lookup entry in one u32. Most ja-JP prefixes are the alphabetic code plus a
