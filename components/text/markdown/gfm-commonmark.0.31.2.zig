...
fn tryExtendedAutolink(out: *Writer, s: []const u8, start: usize) ?usize {
    ...
    if (prefix_len != 0) {
        ...
        if (end > start + prefix_len) {
            writeExtendedURL(out, s[start..end], add_http);
            return end;
        }
    }
...
fn writeInline(out: *Writer, s: []const u8) void {
    ...
    while (i < s.len and !out.overflow) {
        ...
        if (b == '~' and i + 1 < s.len and s[i + 1] == '~') {
            if (std.mem.indexOfPos(u8, s, i + 2, "~~")) |close| {
                if (close > i + 2) {
                    out.writeSlice("<del>");
                    writeInline(out, s[i + 2 .. close]);
                    out.writeSlice("</del>");
                    i = close + 2;
                    continue;
                }
                ...
                if (inner.len > 0) {
                    ...
                    if (c0 == '/' or isAsciiAlpha(c0)) {
                        if (isPlausibleInlineTag(inner)) {
                            writeTagFiltered(out, s[i .. close + 1]);
                            i = close + 1;
                            continue;
                        }
                    } else if (c0 == '!' or c0 == '?') {
        ...
        if (tryExtendedAutolink(out, s, i)) |next| {
            i = next;
            continue;
        }
...
fn renderBlocks(input: []const u8, out: *Writer) void {
    ...
    while (i < lines_count and !out.overflow) {
        ...
        if (renderTable(input, out, &i)) continue;
...
test "GFM task list items render semantic checkboxes" {
    ...
    const size = renderMarkdown(input, &output);
