...
const DFD = [_]u8{
    // One Khronos basic descriptor block for linear Display P3 RGBA32F.
    0x5C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x02, 0x00, 0x58, 0x00, 0x01, 0x0C, 0x01, 0x00,
...
pub fn parse(data: []u8) ?Image {
    ...
    return .{ .width = width, .height = height, .pixels = ptr[0 .. pixel_bytes / 4] };
}
...
test "writes and parses the linear Display P3 profile" {
    var output: [HEADER_SIZE + 2 * 16]u8 align(16) = undefined;
    const size = writeHeader(&output, 2, 1).?;
    ...
    try std.testing.expectEqual(@as(u8, 0x0C), output[117]);
    try std.testing.expectEqual(@as(u8, 0x01), output[118]);
}
