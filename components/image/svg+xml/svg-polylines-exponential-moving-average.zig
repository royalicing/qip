//! Applies an exponential moving average to every `polyline` in the strict
...
//! samples; alpha is `2 / (window_size + 1)`. The component preserves every
//! non-polyline byte and replaces each polyline's raw value-space y values.
...
export fn render(size: u32) packed struct(u64) { output_size_or_failure: u32, output_ptr: u31, failed: u1 } {
    const n: usize = size;
    if (n > INPUT_CAP) @trap();
    const out_n = smoothing.smooth(input[0..n], &output, .exponential_moving_average, window_size) catch {
        window_size = 7;
