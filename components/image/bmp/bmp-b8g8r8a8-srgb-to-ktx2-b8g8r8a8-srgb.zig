//! Wraps QIP's 32-bit sRGB BGRA BMP pixels in a canonical top-down
//! VK_FORMAT_B8G8R8A8_SRGB KTX2 file without changing channel values.
...
export fn render(input_size_in: u32) u32 {
    ...
    while (y < height) : (y += 1) {
        ...
        const source = pixel_offset + source_y * row_bytes;
        const target = ktx.HEADER_SIZE + y * row_bytes;
        @memcpy(output_buf[target .. target + row_bytes], input_buf[source .. source + row_bytes]);
    }
