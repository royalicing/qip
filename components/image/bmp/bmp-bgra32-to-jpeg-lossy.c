...
uint32_t render(uint32_t input_size_value) {
  ...
  uint32_t file_size, height, stride, y;
  ...
  for (y = 0; y < height; ++y) {
    ...
    const uint8_t* source = input_buf + pixel_offset + (size_t)source_y * stride;
    uint32_t x;
    JSAMPROW row = row_buf;
    for (x = 0; x < (uint32_t)width; ++x) {
      uint32_t blue = source[(size_t)x * 4u + 0u];
      uint32_t green = source[(size_t)x * 4u + 1u];
      uint32_t red = source[(size_t)x * 4u + 2u];
