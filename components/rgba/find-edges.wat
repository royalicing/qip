(module $FindEdgesRGBA
  ...
  (func $edge_value
    ...
    (local.set $a00
      (f32.load
        (i32.add
          (i32.add (local.get $row_above_ptr) (local.get $col_left_off))
          (local.get $ch))))
    (local.set $a01
      (f32.load
        (i32.add
          (i32.add (local.get $row_above_ptr) (local.get $col_off))
          (local.get $ch))))
    (local.set $a02
      (f32.load
        (i32.add
          (i32.add (local.get $row_above_ptr) (local.get $col_right_off))
          (local.get $ch))))

    (local.set $a10
      (f32.load
        (i32.add
          (i32.add (local.get $row_ptr) (local.get $col_left_off))
          (local.get $ch))))
    (local.set $a12
      (f32.load
        (i32.add
          (i32.add (local.get $row_ptr) (local.get $col_right_off))
          (local.get $ch))))

    (local.set $a20
      (f32.load
        (i32.add
          (i32.add (local.get $row_below_ptr) (local.get $col_left_off))
          (local.get $ch))))
    (local.set $a21
      (f32.load
        (i32.add
          (i32.add (local.get $row_below_ptr) (local.get $col_off))
          (local.get $ch))))
    (local.set $a22
      (f32.load
        (i32.add
          (i32.add (local.get $row_below_ptr) (local.get $col_right_off))
          (local.get $ch))))
    ...
    (local.set $edge
      (f32.mul (local.get $edge) (f32.const 0.2)))
    (local.set $edge
      (f32.min
        (f32.const 1.0)
        (f32.max (f32.const 0.0) (local.get $edge))))
    (f32.sub (f32.const 1.0) (local.get $edge))
  )
