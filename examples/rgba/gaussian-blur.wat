(module $GaussianBlurRGBA
  ...
  (func $blur_horizontal (param $src_base i32) (param $dst_base i32) (param $radius i32)
    (local $row i32)
    (local $col i32)
    (local $dx i32)
    (local $idx i32)
  ...
  (func $blur_vertical (param $src_base i32) (param $dst_base i32) (param $radius i32)
    (local $row i32)
    (local $col i32)
    (local $dy i32)
    (local $idx i32)
    ...
    (loop $rows
      ...
      (loop $cols
        ...
        (loop $dy_loop
          ...
          (local.set $src_row
            (i32.add (local.get $src_base)
              (i32.mul (local.get $idx) (i32.const 1024))))
          (local.set $p
            (i32.add (local.get $src_row)
              (i32.mul (local.get $col) (i32.const 16))))
          (local.set $sum_r
