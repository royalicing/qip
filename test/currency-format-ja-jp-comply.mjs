...
const complianceBytes = await readFile(
  new URL("../compliance/currency-format-ja-jp.comply.wasm", import.meta.url),
);
...
test("compliance oracle requires ja-JP separators, affixes, and signs", async () => {
  ...
  assert.equal(groupingCase?.expected.toString(), "$1,234.50");
});
...
test("duel: Intl.NumberFormat agrees for every supported ISO 4217 code", async () => {
  ...
  const failures = cases
    ...
    .filter((c) => {
      ...
      const intl = new Intl.NumberFormat("ja-JP", { style: "currency", currency: currency.alpha });
...
test("duel: currency-format-ja-jp.wasm is compliant for every supported currency", async () => {
  const implBytes = await readFile(new URL("../components/text/currency-format-ja-jp.wasm", import.meta.url));
...
test("currency formatter stays within its compact artifact and memory budgets", async () => {
  const implBytes = await readFile(new URL("../components/text/currency-format-ja-jp.wasm", import.meta.url));
  assert.ok(implBytes.byteLength <= 2290, `expected at most 2290 bytes, got ${implBytes.byteLength}`);
