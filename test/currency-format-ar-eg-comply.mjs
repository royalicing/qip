...
const complianceBytes = await readFile(
  new URL("../compliance/currency-format-ar-eg.comply.wasm", import.meta.url),
);
...
test("compliance oracle requires ar-EG digits, grouping, affixes, and signs", async () => {
  ...
  assert.equal(groupingCase?.expected.toString(), "‏١٬٢٣٤٬٥٦٧٫٨٩ US$");
});

test("duel: Intl.NumberFormat ar-EG agrees for every supported ISO 4217 code", async () => {
  ...
  const failures = cases
    ...
    .filter((c) => {
      ...
      const intl = new Intl.NumberFormat("ar-EG", { style: "currency", currency: currency.alpha });
...
test("duel: currency-format-ar-eg.wasm is compliant for every supported currency", async () => {
  const implBytes = await readFile(new URL("../components/utf8/currency-format-ar-eg.wasm", import.meta.url));
...
test("currency formatter stays within its compact artifact and memory budgets", async () => {
  const implBytes = await readFile(new URL("../components/utf8/currency-format-ar-eg.wasm", import.meta.url));
  assert.ok(implBytes.byteLength <= 2700, `expected at most 2700 bytes, got ${implBytes.byteLength}`);
  const { instance } = await WebAssembly.instantiate(implBytes);
