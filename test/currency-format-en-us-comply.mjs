...
const complianceBytes = await readFile(
  new URL("../compliance/currency-format-en-us.comply.wasm", import.meta.url),
);
...
test("duel: Intl.NumberFormat agrees for every supported ISO 4217 code", async () => {
  ...
  const failures = cases
    ...
    .filter((c) => {
      ...
      const intl = new Intl.NumberFormat("en-US", { style: "currency", currency: currency.alpha });
...
test("duel: currency-format-en-us.wasm is compliant for every supported currency", async () => {
  const implBytes = await readFile(new URL("../modules/utf8/currency-format-en-us.wasm", import.meta.url));
