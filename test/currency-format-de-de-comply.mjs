...
const complianceBytes = await readFile(
  new URL("../compliance/currency-format-de-de.comply.wasm", import.meta.url),
);
...
test("compliance oracle requires de-DE digits, grouping, affixes, and signs", async () => {
  ...
  assert.equal(groupingCase?.expected.toString(), "1.234.567,89 $");
});

test("duel: Intl.NumberFormat de-DE agrees for every supported ISO 4217 code", async () => {
  ...
  const failures = cases
    ...
    .filter((c) => {
      ...
      const intl = new Intl.NumberFormat("de-DE", { style: "currency", currency: currency.alpha });
...
test("duel: currency-format-de-de.wasm is compliant for every supported currency", async () => {
  const implBytes = await readFile(new URL("../components/text/currency-format-de-de.wasm", import.meta.url));
