...
const complianceBytes = await readFile(
  new URL("../compliance/currency-format-pt-br.comply.wasm", import.meta.url),
);
...
test("compliance oracle requires pt-BR separators, spacing, affixes, and signs", async () => {
...
test("duel: Intl.NumberFormat agrees for every supported ISO 4217 code", async () => {
  ...
  const failures = cases
    ...
    .filter((c) => {
      ...
      const intl = new Intl.NumberFormat("pt-BR", { style: "currency", currency: currency.alpha });
...
test("duel: currency-format-pt-br.wasm is compliant for every supported currency", async () => {
  const implBytes = await readFile(new URL("../components/utf8/currency-format-pt-br.wasm", import.meta.url));
...
test("currency formatter stays within its compact artifact and memory budgets", async () => {
  const implBytes = await readFile(new URL("../components/utf8/currency-format-pt-br.wasm", import.meta.url));
  assert.ok(implBytes.byteLength <= 2330, `expected at most 2330 bytes, got ${implBytes.byteLength}`);
