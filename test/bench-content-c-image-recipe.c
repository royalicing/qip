...
#ifndef QIP_IMAGE_BUFFER_CAPACITY
#define QIP_IMAGE_BUFFER_CAPACITY (128u * 1024u * 1024u)
#endif
...
#define RUN_STEP(number, input_bytes, input_length, output_bytes)                 \
    do {                                                                          \
        ...
        if (STEP_##number##_RENDER(                                               \
                ...
                &offset,                                                          \
                &n) != 0) return 0;                                               \
        if (n > QIP_IMAGE_BUFFER_CAPACITY) return 0;                              \
        memcpy(                                                                   \
            output_bytes,                                                         \
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr, "usage: bench-content-c-image-recipe input duration-ms [output]\n");
        return 2;
    }
    memset(&r, 0, sizeof(r));
    ...
    buffer_a = malloc(QIP_IMAGE_BUFFER_CAPACITY);
    buffer_b = malloc(QIP_IMAGE_BUFFER_CAPACITY);
    ...
    printf(
        "{\"runtime\":\"generated-c-image-recipe\",\"memory\":\"%s\","
        "\"steps\":3,\"input_bytes\":%ld,\"output_bytes\":%u,"
        "\"arena_bytes\":%llu,\"dedicated_memory_bytes\":%llu,"
        "\"host_buffer_capacity_bytes\":%llu,"
        ...
        (unsigned long long)QIP_IMAGE_BUFFER_CAPACITY * 2,
