// Content Compliance component: en-IN currency formatting selected by ISO
...
// integer digits. The selected country currency supplies a CLDR-48 en-IN
// prefix and fraction-digit count. Rounding is half away from zero. Invalid
// input traps.
const currency_data = @import("currency-format-en-in-table.zig");
...
fn oracle(input: []const u8, output: []u8) ?usize {
    ...
    for (digits_buf[0..digits_len], 0..) |digit, i| {
        const remaining = digits_len - i;
        if (i != 0 and (remaining == 3 or (remaining > 3 and (remaining - 3) % 2 == 0))) {
            output[out] = ',';
            out += 1;
        }
        output[out] = digit;
