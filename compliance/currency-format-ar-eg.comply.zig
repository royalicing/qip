// Content Compliance component: ar-EG currency formatting selected by ISO
...
// integer digits. The selected country currency supplies a CLDR-48 ar-EG
// suffix and fraction-digit count. Rounding is half away from zero. Invalid
// input traps.
const currency_data = @import("currency-format-ar-eg-table.zig");
...
fn writeArabicDigit(output: []u8, out: *usize, digit: u8) void {
    output[out.*] = 0xd9;
    output[out.* + 1] = 0xa0 + (digit - '0');
    out.* += 2;
}
...
fn oracle(input: []const u8, output: []u8) ?usize {
    ...
    var out: usize = 0;
    const sign_prefix = if (parsed.negative) "\xD8\x9C-\xE2\x80\x8F" else "\xE2\x80\x8F";
    for (sign_prefix) |byte| {
        output[out] = byte;
        out += 1;
    }
    for (digits_buf[0..digits_len], 0..) |digit, i| {
        ...
        writeArabicDigit(output, &out, digit);
    }
    if (currency.fraction_digits > 0) {
        ...
        position = 0;
        while (position < currency.fraction_digits) : (position += 1) {
            const digit = '0' + @as(u8, @intCast((fraction_value / divisor) % 10));
            writeArabicDigit(output, &out, digit);
