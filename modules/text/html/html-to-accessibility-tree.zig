...
fn setAttribute(element: *Element, name: []const u8, value: Range) void {
    if (eqlIgnoreCase(name, "id")) element.id = value else if (eqlIgnoreCase(name, "role")) element.role = value else if (eqlIgnoreCase(name, "aria-label")) element.aria_label = value else if (eqlIgnoreCase(name, "aria-labelledby")) element.aria_labelledby = value else if (eqlIgnoreCase(name, "aria-hidden")) element.aria_hidden = value else if (eqlIgnoreCase(name, "type")) element.input_type = value else if (eqlIgnoreCase(name, "alt")) element.alt = value else if (eqlIgnoreCase(name, "value")) element.value = value else if (eqlIgnoreCase(name, "title")) element.title = value else if (eqlIgnoreCase(name, "placeholder")) element.placeholder = value else if (eqlIgnoreCase(name, "for")) element.label_for = value else if (eqlIgnoreCase(name, "href")) element.href_present = true else if (eqlIgnoreCase(name, "hidden")) element.hidden_present = true;
}
...
fn implicitRole(element_index: usize, input: []const u8) RoleInfo {
    ...
    var label: []const u8 = "";
    if ((isTagBytes(tag, "a") or isTagBytes(tag, "area")) and element.href_present) label = "link" else if (isTagBytes(tag, "button") or isTagBytes(tag, "summary")) label = "button" else if (isTagBytes(tag, "textarea")) label = "textbox" else if (isTagBytes(tag, "select")) label = "combobox" else if (isTagBytes(tag, "option")) label = "option" else if (isTagBytes(tag, "ul") or isTagBytes(tag, "ol") or isTagBytes(tag, "menu")) label = "list" else if (isTagBytes(tag, "li")) label = "listitem" else if (isTagBytes(tag, "main")) label = "main" else if (isTagBytes(tag, "nav")) label = "navigation" else if (isTagBytes(tag, "header") and !hasSectioningAncestor(element_index, input)) label = "banner" else if (isTagBytes(tag, "footer") and !hasSectioningAncestor(element_index, input)) label = "contentinfo" else if (isTagBytes(tag, "section") and hasDirectAuthorName(element_index, input)) label = "region" else if (isTagBytes(tag, "article")) label = "article" else if (isTagBytes(tag, "aside")) label = "complementary" else if (isTagBytes(tag, "form") and hasDirectAuthorName(element_index, input)) label = "form" else if (isTagBytes(tag, "fieldset")) label = "group" else if (isTagBytes(tag, "figure")) label = "figure" else if (isTagBytes(tag, "table")) label = "table" else if (isTagBytes(tag, "tr")) label = "row" else if (isTagBytes(tag, "th")) label = "columnheader" else if (isTagBytes(tag, "td")) label = "cell" else if (isTagBytes(tag, "dialog")) label = "dialog" else if (isTagBytes(tag, "p")) label = "paragraph" else if (isTagBytes(tag, "h1") or isTagBytes(tag, "h2") or isTagBytes(tag, "h3") or isTagBytes(tag, "h4") or isTagBytes(tag, "h5") or isTagBytes(tag, "h6")) label = "heading" else if (isTagBytes(tag, "img")) {
...
noinline fn computeName(element_index: usize, role: RoleInfo, out: *NameBuffer, input: []const u8, element_count: usize) void {
    ...
    const is_form_control = is_input or isTagBytes(tag, "textarea") or isTagBytes(tag, "select") or isTagBytes(tag, "button");
